...
const ChatRoom = () => {
  ...
  useEffect(() => {
    const fetchChatRooms = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get('/api/chat/rooms/me', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        if (message.body) {
          ...
          setMessages((prev) => {
            const isDuplicate = prev.some(
              (msg) =>
                ...
                msg.messageType === receivedMessage.messageType &&
                msg.socialId === receivedMessage.socialId &&
                msg.chatRole === receivedMessage.chatRole
            );
            console.log(
              'Received message:',
              receivedMessage.chatRole,
              receivedMessage.content,
              receivedMessage.sender,
              receivedMessage.socialId
            );
          ...
          (async () => {
            try {
              ...
              await axios.post(
                `/api/chatrooms/${roomId}/read?userId=${userId}`,
                {},
                {
                  headers: {
                    Authorization: `Bearer ${token}`,
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const res = await fetch(`/api/chat/upload/${roomId}`, {
        method: 'POST',
        body: formData,
        headers: {
  ...
  const fetchChatRooms = async (userId: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('/api/chat/with-unread', {
        params: { userId },
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      <div className="flex flex-col w-3/4 border-r bg-white">
                ...
                {!isMine && (
                  <div className="flex items-center mb-1 ml-2 space-x-2">
                    ...
                    {message.chatRole === 'MENTOR' && (
                      <span className="text-xs bg-green-500 text-white rounded px-1">
                        멘토
                      </span>
                    )}
                  </div>
        ...
        <div className="flex items-center p-4 border-t">
          ...
          <label
            ...
            className="ml-2 cursor-pointer text-white bg-purple-500 px-4 py-2 rounded-lg select-none"
          >
            📷
          </label>
          <input
